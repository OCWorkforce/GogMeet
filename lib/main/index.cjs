...
const SWIFT_SRC_DEV = (0, external_node_path_namespaceObject.join)(calendar_dirname, '..', '..', 'src', 'main', 'googlemeet-events.swift');
const isPackaged = calendar_dirname.includes('.asar');
const BINARY_DIR = (0, external_node_path_namespaceObject.join)((0, external_node_os_namespaceObject.tmpdir)(), 'googlemeet');
const BINARY_PATH = (0, external_node_path_namespaceObject.join)(BINARY_DIR, 'googlemeet-events');
...
async function ensureBinary() {
    const swiftSrc = isPackaged ? (0, external_node_path_namespaceObject.join)(process.resourcesPath, 'app.asar.unpacked', 'src', 'main', 'googlemeet-events.swift') : SWIFT_SRC_DEV;
...
function formatMeetingTime(isoDate) {
...
let tray = null;
function setupTray(_win) {
    const assetsDir = external_node_path_default().join(tray_dirname, "..", "..", "src", "assets");
    function buildIcon(isDark) {
        const suffix = isDark ? "dark" : "light";
        const icon1x = external_electron_namespaceObject.nativeImage.createFromPath(external_node_path_default().join(assetsDir, `tray-icon-${suffix}.png`));
        const icon2x = external_electron_namespaceObject.nativeImage.createFromPath(external_node_path_default().join(assetsDir, `tray-icon-${suffix}@2x.png`));
    ...
    tray = new external_electron_namespaceObject.Tray(buildIcon(external_electron_namespaceObject.nativeTheme.shouldUseDarkColors));
    tray.setToolTip("Google Meet");
    const onThemeUpdated = ()=>{
        tray?.setImage(buildIcon(external_electron_namespaceObject.nativeTheme.shouldUseDarkColors));
    };
    external_electron_namespaceObject.nativeTheme.on("updated", onThemeUpdated);
    function buildMeetingMenuTemplate(events) {
        const now = new Date();
        ...
        if (todayEvents.length > 0) {
            ...
            for (const event of todayEvents)items.push({
                label: `${event.title}  –  ${formatMeetingTime(event.startDate)}`,
                click: ()=>{
                    external_electron_namespaceObject.shell.openExternal(buildMeetUrl(event));
                }
            });
        }
        if (tomorrowEvents.length > 0) {
            ...
            for (const event of tomorrowEvents)items.push({
                label: `${event.title}  –  ${formatMeetingTime(event.startDate)}`,
                click: ()=>{
                    external_electron_namespaceObject.shell.openExternal(buildMeetUrl(event));
                }
    ...
    tray.on("click", async ()=>{
        ...
        ] : buildMeetingMenuTemplate(result.events);
        tray.popUpContextMenu(external_electron_namespaceObject.Menu.buildFromTemplate(template));
    });
}
...
let activeTitleEventId = null;
let consecutiveErrors = 0;
const MAX_CONSECUTIVE_ERRORS = 3;
let pollInterval = null;
function resolveActiveTitleEvent() {
    let bestId = null;
    let bestStartMs = 1 / 0;
    ...
    if (bestId) {
        ...
        const remaining = Math.ceil((data.startMs - Date.now()) / 60000);
        if (remaining > 0) updateTrayTitle(data.title, remaining);
    } else updateTrayTitle(null);
}
function scheduleEvents(events) {
    ...
    for (const event of events){
        ...
        if (timers.has(event.id)) {
            ...
            if (prevData) {
                ...
                if (!timeChanged && !titleChanged && !urlChanged) continue;
                if (timeChanged) {
        ...
        function startCountdown() {
            ...
            const clearHandle = setTimeout(()=>{
                ...
                countdownIntervals.delete(event.id);
                clearTimers.delete(event.id);
                if (activeTitleEventId === event.id) activeTitleEventId = null;
                resolveActiveTitleEvent();
                console.log(`[scheduler] Tray title updated (meeting started: "${event.title}")`);
    ...
    for (const id of scheduledEventData.keys())if (!activeIds.has(id)) scheduledEventData.delete(id);
    resolveActiveTitleEvent();
}
async function poll() {
    try {
...
process.on("uncaughtException", (error)=>{
    ...
    if (!isDev) {
        external_electron_namespaceObject.dialog.showErrorBox("Unexpected Error", error.message || "An unexpected error occurred.");
        external_electron_namespaceObject.app.exit(1);
    }
});
