    ...
    for (const event of events) {
        ...
        if (timers.has(event.id)) {
            ...
            if (prevData) {
                ...
                if (!timeChanged && !titleChanged && !urlChanged)
        ...
        function tickCountdown() {
            ...
            if (event.id !== activeTitleEventId)
                return;
        ...
        function startCountdown() {
            ...
            if (!scheduledEventData.has(event.id))
                return;
    ...
    try {
        ...
        if ("events" in result) {
            ...
            if (consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
                ...
                for (const handle of clearTimers.values())
                    clearTimeout(handle);
                clearTimers.clear();
                resolveActiveTitleEvent();
...
export function stopScheduler() {
    ...
    firedEvents.clear();
    for (const handle of titleTimers.values())
        clearTimeout(handle);
    titleTimers.clear();
    for (const handle of countdownIntervals.values())
        clearInterval(handle);
...
export function _resetConsecutiveErrors() {
    consecutiveErrors = 0;
}
