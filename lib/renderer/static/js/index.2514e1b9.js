(()=>{"use strict";let e={type:"loading"},t="",n=null;function s(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function a(){var n,a,o,l,d;let c=document.getElementById("app");c&&(c.innerHTML=`
    <div class="header">
  ...
  `,null==(n=document.getElementById("btn-refresh"))||n.addEventListener("click",()=>r()),null==(a=document.getElementById("footer-refresh"))||a.addEventListener("click",()=>r()),null==(o=document.getElementById("btn-close"))||o.addEventListener("click",()=>{let e=document.getElementById("app");e?(e.classList.add("hiding"),e.addEventListener("transitionend",()=>window.api.window.minimizeToTray(),{once:!0})):window.api.window.minimizeToTray()}),null==(l=document.getElementById("btn-grant"))||l.addEventListener("click",()=>i()),null==(d=document.getElementById("btn-retry"))||d.addEventListener("click",()=>r()),document.querySelectorAll(".btn-join").forEach(e=>{e.addEventListener("click",()=>{let t=e.dataset.url;t&&window.api.app.openExternal(t)})}))}async function i(){e={type:"no-permission",retrying:!0},a(),"granted"===await window.api.calendar.requestPermission()?await r():(e={type:"no-permission",retrying:!1},a())}async function r(){e={type:"loading"},a();try{let t=await window.api.calendar.getPermissionStatus();if("denied"===t||"not-determined"===t){e={type:"no-permission",retrying:!1},a();return}let n=await window.api.calendar.getEvents();e=0===n.length?{type:"no-events"}:{type:"has-events",events:n}}catch(t){e={type:"error",message:t instanceof Error?t.message:"Unknown error"}}a()}async function o(){t=await window.api.app.getVersion(),await r(),n&&clearInterval(n),n=setInterval(()=>r(),3e5)}document.addEventListener("DOMContentLoaded",()=>o())})();